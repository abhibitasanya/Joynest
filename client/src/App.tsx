...
function FloatingPolaroid({ data }: { data: typeof polaroidData[0] }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-center mt-2 text-xs" style={{ fontFamily: 'cursive', color: '#8A7F74', lineHeight: 1.3 }}>{data.label}</p>
    </div>
  )
}
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        >
          <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #1C3A2A, #2D5A3F)' }}>
            <span className="text-sm">🪺</span>
          </div>
          <span className="font-serif text-xl font-semibold" style={{ color: '#1C3A2A' }}>JoyNest</span>
        </button>

        <div className="hidden md:flex items-center gap-8">
          {['Templates', 'Features', 'Pricing', 'Gallery'].map(item => (
            <a key={item} href="#" className="text-sm font-medium transition-colors duration-200 hover:text-[#C9A84C]" style={{ color: '#8A7F74' }}>
              {item}
            </a>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={onAuth}
            className="text-sm font-medium px-4 py-2 rounded-full transition-all duration-200 hover:bg-[#F5EDD9]"
            style={{ color: '#1C3A2A' }}
          >
            Sign in
          </button>
...
function HeroSection({ onStart }: { onStart: () => void }) {
  ...
  return (
    <section
      ...
      style={{ background: 'linear-gradient(165deg, #FAF7F2 0%, #F5EDD9 40%, #FAF7F2 70%, #EFF5EF 100%)' }}
    >
      {/* Ambient blobs */}
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute w-80 h-80 rounded-full blur-3xl opacity-20"
          style={{ background: 'radial-gradient(circle, #B76E79 0%, transparent 70%)', bottom: '20%', right: '20%',
          animation: 'pulse-soft 4s ease-in-out 1.5s infinite' }} />
        <div className="absolute w-64 h-64 rounded-full blur-3xl opacity-20"
          style={{ background: 'radial-gradient(circle, #8FAF8F 0%, transparent 70%)', top: '50%', right: '30%',
      ...
      <div className="relative z-10 text-center max-w-4xl mx-auto px-6">
        <div className="animate-fadeInUp delay-100 inline-flex items-center gap-2 px-4 py-2 rounded-full mb-8"
          ...
          <span className="text-xs font-medium tracking-wider uppercase" style={{ color: '#C9A84C' }}>Memory making, reimagined</span>
        </div>

        <h1 className="font-serif animate-fadeInUp delay-200" style={{ fontSize: 'clamp(3rem, 7vw, 6.5rem)', lineHeight: 1.05, color: '#1C3A2A', letterSpacing: '-0.02em' }}>
          Preserve Moments.<br />
          <em style={{ color: '#C9A84C', fontStyle: 'italic' }}>Share Memories.</em><br />
          Create Joy.
        </h1>

        <p className="animate-fadeInUp delay-300 mt-6 text-lg leading-relaxed max-w-xl mx-auto"
          style={{ color: '#8A7F74', fontWeight: 300 }}>
        ...
        <div className="animate-fadeInUp delay-400 flex flex-col sm:flex-row gap-4 justify-center mt-10">
          <button
            onClick={onStart}
            className="btn-primary font-medium px-8 py-4 rounded-full text-base"
          >
            <span>Create Your Memory</span>
          </button>
          <button className="btn-gold font-medium px-8 py-4 rounded-full text-base">
            Explore Templates
          </button>
          <button className="flex items-center gap-2 px-6 py-4 rounded-full font-medium text-base transition-all duration-200 hover:bg-[#F5EDD9]"
            style={{ color: '#1C3A2A', border: '1px solid rgba(28,58,42,0.15)' }}>
            <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs"
              style={{ background: '#1C3A2A', color: 'white' }}>▶</span>
            Watch Demo
          </button>
        </div>

        <div className="animate-fadeInUp delay-500 flex items-center justify-center gap-6 mt-12">
          {[['10k+', 'Memories created'], ['98%', 'Love it'], ['4.9★', 'App rating']].map(([n, l]) => (
            <div key={l} className="text-center">
              <div className="font-serif text-2xl font-semibold" style={{ color: '#1C3A2A' }}>{n}</div>
              <div className="text-xs mt-0.5" style={{ color: '#8A7F74' }}>{l}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Scroll indicator */}
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 animate-bounce opacity-50" style={{ color: '#8A7F74' }}>
...
// ─── Features Section ─────────────────────────────────────────────────────────
const features = [
  { icon: '📖', title: 'Interactive Journals', desc: 'Flip through beautifully crafted pages with handwritten fonts, stickers, and washi tape accents.', color: '#F5EDD9' },
  { icon: '🎙️', title: 'Voice Notes', desc: 'Capture the warmth of a voice. Record messages that play as visitors scroll through your story.', color: '#EFF5EF' },
  { icon: '🖼️', title: 'Memory Gallery', desc: 'A cinematic masonry gallery with lightbox, captions, albums, and seamless drag-to-upload.', color: '#FAF0F2' },
  { icon: '🎵', title: 'Background Music', desc: 'Set the mood with a curated playlist or your own tracks. Vinyl player animations included.', color: '#F5EDD9' },
  { icon: '🔐', title: 'Password Protection', desc: 'Keep private memories safe with elegant password protection and invite-only access.', color: '#EFF5EF' },
  { icon: '⏳', title: 'Memory Timeline', desc: 'Tell your story chronologically with a beautiful interactive timeline that unfolds as you scroll.', color: '#FAF0F2' },
]

function FeaturesSection() {
  return (
    <section className="py-32 px-6" style={{ background: '#FAF7F2' }}>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <p className="text-sm font-medium tracking-widest uppercase mb-4" style={{ color: '#C9A84C' }}>Why JoyNest</p>
          <h2 className="font-serif" style={{ fontSize: 'clamp(2.2rem, 4vw, 3.5rem)', color: '#1C3A2A', lineHeight: 1.1 }}>
            Everything a memory deserves
          </h2>
          <p className="mt-4 text-base max-w-xl mx-auto" style={{ color: '#8A7F74', fontWeight: 300 }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((f, i) => (
            ...
            >
              <div className="text-4xl mb-5 transform transition-transform duration-300 group-hover:scale-110 inline-block">{f.icon}</div>
              <h3 className="font-serif text-xl font-semibold mb-2" style={{ color: '#1C3A2A' }}>{f.title}</h3>
              <p className="text-sm leading-relaxed" style={{ color: '#8A7F74' }}>{f.desc}</p>
              <div className="mt-4 flex items-center gap-1 text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ color: '#C9A84C' }}>
...
// ─── Templates Section ────────────────────────────────────────────────────────
const templates = [
  { name: 'Birthday Delight', tag: 'birthday', img: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400&h=280&fit=crop&auto=format', palette: ['#B76E79', '#F5EDD9', '#C9A84C'] },
  { name: 'Wedding Story', tag: 'wedding', img: 'https://images.unsplash.com/photo-1519741497674-611481863552?w=400&h=280&fit=crop&auto=format', palette: ['#FAF7F2', '#C9A84C', '#8A7F74'] },
  { name: 'Love Letters', tag: 'romance', img: 'https://images.unsplash.com/photo-1516589178581-6cd7833ae3b2?w=400&h=280&fit=crop&auto=format', palette: ['#FAF0F2', '#B76E79', '#F5EDD9'] },
  { name: 'Travel Diaries', tag: 'travel', img: 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=400&h=280&fit=crop&auto=format', palette: ['#EFF5EF', '#1C3A2A', '#C9A84C'] },
  { name: 'Graduation Day', tag: 'graduation', img: 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?w=400&h=280&fit=crop&auto=format', palette: ['#F5EDD9', '#2D5A3F', '#C9A84C'] },
  { name: 'Family Album', tag: 'family', img: 'https://images.unsplash.com/photo-1529156069898-49953e39b3ac?w=400&h=280&fit=crop&auto=format', palette: ['#FAF7F2', '#8FAF8F', '#C5B8D4'] },
]

function TemplatesSection({ onStart }: { onStart: () => void }) {
  const [hovered, setHovered] = useState<number | null>(null)

  return (
    <section className="py-32 px-6" style={{ background: 'linear-gradient(180deg, #FAF7F2 0%, #F5EDD9 100%)' }}>
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div>
            <p className="text-sm font-medium tracking-widest uppercase mb-4" style={{ color: '#C9A84C' }}>Templates</p>
            <h2 className="font-serif" style={{ fontSize: 'clamp(2.2rem, 4vw, 3.5rem)', color: '#1C3A2A', lineHeight: 1.1 }}>
          ...
          <button className="self-start md:self-auto text-sm font-medium px-6 py-3 rounded-full transition-all duration-200"
            style={{ border: '1px solid rgba(28,58,42,0.2)', color: '#1C3A2A' }}
            onMouseEnter={e => (e.currentTarget.style.background = '#1C3A2A', e.currentTarget.style.color = '#FAF7F2')}
            onMouseLeave={e => (e.currentTarget.style.background = 'transparent', e.currentTarget.style.color = '#1C3A2A')}>
            View all templates →
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {templates.map((t, i) => (
            <div
              ...
              style={{ boxShadow: hovered === i ? '0 24px 60px rgba(28,58,42,0.14)' : '0 4px 20px rgba(28,58,42,0.06)',
            ...
            >
              <div className="relative overflow-hidden" style={{ height: 200 }}>
                ...
                <div className="absolute inset-0 flex items-center justify-center gap-3 transition-all duration-300"
                  ...
                  <button className="text-xs font-semibold px-4 py-2 rounded-full transition-all duration-300"
                    style={{ background: 'rgba(250,247,242,0.9)', color: '#1C3A2A',
                      transform: hovered === i ? 'translateY(0)' : 'translateY(8px)', transition: 'transform 0.3s ease 0.1s' }}>
                    Preview
                  </button>
                </div>
              </div>

              <div className="p-5" style={{ background: '#FAF7F2' }}>
                <div className="flex items-center justify-between">
                  <h3 className="font-serif font-semibold text-base" style={{ color: '#1C3A2A' }}>{t.name}</h3>
                  <span className="text-xs px-2.5 py-1 rounded-full font-medium capitalize"
                    style={{ background: '#F5EDD9', color: '#8A7F74' }}>{t.tag}</span>
                </div>
              </div>
            </div>
          ))}
...
// ─── Testimonials ─────────────────────────────────────────────────────────────
const testimonials = [
  { name: 'Amara Osei', role: 'Made a birthday site for her mother', quote: "She cried. I cried. Everyone cried. JoyNest turned a few photos and words into something I'll treasure forever.", avatar: 'https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?w=80&h=80&fit=crop&auto=format' },
  { name: 'Luca Ferretti', role: 'Wedding anniversary gift', quote: "My wife said it was the most romantic thing I'd ever done. The page-flip journals and voice notes made it feel like magic.", avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=80&h=80&fit=crop&auto=format' },
  { name: 'Yuna Tanaka', role: 'Graduation surprise for friends', quote: "We built a surprise site for our whole friend group. The gallery and music made it feel like a film. So much love in one place.", avatar: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?w=80&h=80&fit=crop&auto=format' },
]

function TestimonialsSection() {
  return (
    <section className="py-32 px-6" style={{ background: '#1C3A2A' }}>
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <p className="text-sm font-medium tracking-widest uppercase mb-4" style={{ color: '#C9A84C' }}>Loved by thousands</p>
          <h2 className="font-serif text-white" style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', lineHeight: 1.1 }}>
            Moments that made people<br /><em style={{ color: '#C9A84C' }}>smile</em>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((t) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <img src={t.avatar} alt={t.name} className="w-10 h-10 rounded-full object-cover" style={{ border: '2px solid rgba(201,168,76,0.4)' }} />
                <div>
                  <div className="font-semibold text-sm" style={{ color: '#FAF7F2' }}>{t.name}</div>
...
function CTASection({ onStart }: { onStart: () => void }) {
  return (
    <section className="py-40 px-6 text-center relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #F5EDD9 0%, #FAF7F2 50%, #EFF5EF 100%)' }}>
      <div className="absolute inset-0 pointer-events-none opacity-30">
        <div className="absolute w-96 h-96 rounded-full blur-3xl" style={{ background: 'radial-gradient(circle, #C9A84C, transparent 70%)', top: '-20%', left: '-10%' }} />
        <div className="absolute w-80 h-80 rounded-full blur-3xl" style={{ background: 'radial-gradient(circle, #B76E79, transparent 70%)', bottom: '-20%', right: '-5%' }} />
      </div>
      <div className="relative max-w-3xl mx-auto">
        <div className="text-6xl mb-6">🪺</div>
        <h2 className="font-serif mb-4" style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)', color: '#1C3A2A', lineHeight: 1.05 }}>
          Your next memory<br />is waiting to be made
        </h2>
        <p className="text-base mb-10 max-w-md mx-auto" style={{ color: '#8A7F74', fontWeight: 300 }}>
          Free to start. No design skills needed. Just love, stories, and a little bit of magic.
        </p>
        <button
          onClick={onStart}
          className="btn-primary font-semibold px-10 py-4 rounded-full text-lg"
        >
...
function Footer() {
  return (
    <footer className="py-16 px-6" style={{ background: '#1C3A2A', borderTop: '1px solid rgba(201,168,76,0.1)' }}>
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-8 mb-12">
          ...
          {[
            ...
            <div key={col.title}>
              <h4 className="font-semibold text-sm mb-4" style={{ color: '#FAF7F2' }}>{col.title}</h4>
              <ul className="space-y-2.5">
                {col.links.map(link => (
                  <li key={link}>
                    <a href="#" className="text-sm transition-colors duration-200 hover:text-[#C9A84C]"
        ...
        <div className="flex flex-col md:flex-row items-center justify-between pt-8"
          ...
          <p className="text-sm" style={{ color: 'rgba(250,247,242,0.35)' }}>© 2025 JoyNest. Made with love and a little magic.</p>
          <div className="flex gap-4 mt-4 md:mt-0">
            {['Twitter', 'Instagram', 'Pinterest'].map(s => (
              <a key={s} href="#" className="text-sm transition-colors duration-200 hover:text-[#C9A84C]"
      ...
      <div className="w-full max-w-4xl rounded-3xl overflow-hidden animate-scaleIn"
        ...
        <div className="grid md:grid-cols-2">
          ...
          <div className="relative hidden md:flex flex-col items-center justify-center p-12 overflow-hidden"
            style={{ background: 'linear-gradient(135deg, #1C3A2A 0%, #2D5A3F 100%)' }}>
            <div className="absolute inset-0 opacity-20">
              <div className="absolute w-40 h-40 rounded-full blur-2xl" style={{ background: '#C9A84C', top: '10%', right: '10%', animation: 'pulse-soft 4s ease-in-out infinite' }} />
              <div className="absolute w-32 h-32 rounded-full blur-2xl" style={{ background: '#B76E79', bottom: '20%', left: '5%', animation: 'pulse-soft 5s ease-in-out 1s infinite' }} />
            </div>
            ...
            <div className="relative z-10 text-center">
              <div className="relative w-48 mx-auto mb-8">
                <div className="polaroid" style={{ transform: 'rotate(-6deg)', marginBottom: '-20px', position: 'relative', zIndex: 1 }}>
                  ...
                  <p className="text-center text-xs mt-2" style={{ fontFamily: 'cursive', color: '#8A7F74' }}>Forever friends 💛</p>
                </div>
                <div className="polaroid absolute top-4 left-4" style={{ transform: 'rotate(4deg)', zIndex: 2 }}>
                  ...
                  <p className="text-center text-xs mt-2" style={{ fontFamily: 'cursive', color: '#8A7F74' }}>Just married ✨</p>
                </div>
              </div>
          ...
          {/* Right panel */}
          <div className="p-10 md:p-12 paper-texture" style={{ background: '#FAF7F2' }}>
            <button onClick={onClose} className="float-right w-8 h-8 flex items-center justify-center rounded-full transition-colors hover:bg-[#F5EDD9] text-lg" style={{ color: '#8A7F74' }}>×</button>
            ...
            <div className="mb-8">
              ...
              <h2 className="font-serif text-3xl font-semibold mb-1" style={{ color: '#1C3A2A' }}>
                {mode === 'login' ? 'Welcome back' : 'Start your story'}
              </h2>
              <p className="text-sm" style={{ color: '#8A7F74' }}>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="text-xs" style={{ color: '#8A7F74' }}>or continue with email</span>
              <div className="flex-1 h-px" style={{ background: 'rgba(28,58,42,0.1)' }} />
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                />
                {mode === 'login' && (
                  <a href="#" className="block text-right text-xs mt-1.5 hover:text-[#C9A84C] transition-colors" style={{ color: '#8A7F74' }}>Forgot password?</a>
                )}
              </div>
            </div>
            ...
            <button
              onClick={onSuccess}
              className="btn-primary w-full font-semibold py-3.5 rounded-xl mt-6 text-sm"
            >
              <span>{mode === 'login' ? 'Sign in to JoyNest' : 'Create my account'}</span>
            </button>

            <p className="text-center text-sm mt-5" style={{ color: '#8A7F74' }}>
              {mode === 'login' ? "Don't have an account? " : 'Already have an account? '}
              <button onClick={() => setMode(mode === 'login' ? 'signup' : 'login')}
                className="font-semibold hover:text-[#C9A84C] transition-colors" style={{ color: '#1C3A2A' }}>
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#FAF7F2' }}>
      ...
      <aside className="w-64 hidden lg:flex flex-col fixed top-0 left-0 h-screen z-40"
        style={{ background: '#1C3A2A', borderRight: '1px solid rgba(201,168,76,0.1)' }}>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                color: item.active ? '#C9A84C' : 'rgba(250,247,242,0.6)',
              }}
        ...
        <div className="p-4 border-t" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
          <div className="rounded-xl p-4" style={{ background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.2)' }}>
            <p className="text-xs font-medium mb-1" style={{ color: '#C9A84C' }}>Free Plan</p>
            ...
            <div className="w-full h-1.5 rounded-full mb-3" style={{ background: 'rgba(255,255,255,0.1)' }}>
              <div className="h-full rounded-full" style={{ width: '66%', background: 'linear-gradient(90deg, #C9A84C, #E2C97E)' }} />
            </div>
            <button className="w-full text-xs font-semibold py-2 rounded-lg transition-all duration-200"
              style={{ background: 'linear-gradient(135deg, #C9A84C, #E2C97E)', color: '#1C3A2A' }}>
              Upgrade to Pro
            </button>
          </div>
          ...
          <div className="flex items-center gap-3 mt-4 px-1">
            ...
            <div>
              <div className="text-sm font-medium" style={{ color: '#FAF7F2' }}>Alex Rivera</div>
      ...
      <main className="flex-1 lg:ml-64 min-h-screen">
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between px-6 lg:px-10 py-4"
          ...
          <div>
            <h1 className="font-serif text-xl font-semibold" style={{ color: '#1C3A2A' }}>
              {greeting}, Alex. ✨
            </h1>
            <p className="text-xs" style={{ color: '#8A7F74' }}>Continue creating memories</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="relative hidden md:block">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm" style={{ color: '#8A7F74' }}>🔍</span>
              <input
                placeholder="Search memories..."
                className="pl-9 pr-4 py-2 text-sm rounded-full outline-none"
                style={{ background: '#F5EDD9', border: '1px solid rgba(28,58,42,0.08)', color: '#2A2A2A', width: 200 }}
              />
            </div>
            <button
        ...
        <div className="px-6 lg:px-10 py-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
            {[
              { label: 'Total projects', val: '4', icon: '📁', color: '#F5EDD9' },
              { label: 'Total views', val: '298', icon: '👁', color: '#EFF5EF' },
              { label: 'Shared links', val: '3', icon: '🔗', color: '#FAF0F2' },
              { label: 'Photos stored', val: '127', icon: '🖼️', color: '#F5EDD9' },
            ].map(s => (
              <div key={s.label} className="rounded-2xl p-5 card-hover" style={{ background: s.color, border: '1px solid rgba(28,58,42,0.05)' }}>
                <div className="text-2xl mb-2">{s.icon}</div>
                <div className="font-serif text-3xl font-semibold" style={{ color: '#1C3A2A' }}>{s.val}</div>
                <div className="text-xs mt-0.5" style={{ color: '#8A7F74' }}>{s.label}</div>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between mb-6">
            <h2 className="font-serif text-2xl font-semibold" style={{ color: '#1C3A2A' }}>My Memories</h2>
            <div className="flex gap-1 p-1 rounded-xl" style={{ background: '#F5EDD9' }}>
              {['all', 'published', 'draft'].map(tab => (
                <button
                  ...
                  style={{
                    background: activeTab === tab ? '#1C3A2A' : 'transparent',
                    color: activeTab === tab ? '#FAF7F2' : '#8A7F74',
                  }}
                >
                  {tab}
                </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-6">
              ...
              >
                <div className="relative overflow-hidden" style={{ height: 180 }}>
                  ...
                  <div className="absolute top-3 left-3">
                    <span className="text-xs px-2.5 py-1 rounded-full font-medium capitalize"
                      style={{
                        ...
                        color: p.status === 'published' ? '#C9A84C' : '#1C3A2A',
                      }}>
                  ...
                  <div className="absolute top-3 right-3 flex gap-2 transition-all duration-300"
                    ...
                    <button
                      ...
                      style={{ background: 'rgba(250,247,242,0.95)', color: '#1C3A2A' }}>
                      Edit
                    </button>
                    <button className="text-xs font-medium px-3 py-1.5 rounded-full transition-all duration-200"
                      style={{ background: 'rgba(201,168,76,0.9)', color: '#1C3A2A' }}>
                      Share
                    </button>
                  </div>
                </div>
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <h3 className="font-serif font-semibold text-base" style={{ color: '#1C3A2A' }}>{p.title}</h3>
                      <div className="flex items-center gap-3 mt-1.5">
                        <span className="text-xs capitalize" style={{ color: '#8A7F74' }}>{p.type}</span>
                        <span className="text-xs" style={{ color: '#C5D9C5' }}>·</span>
                        <span className="text-xs" style={{ color: '#8A7F74' }}>Edited {p.edited}</span>
                      </div>
                    </div>
                    {p.views > 0 && (
                      <div className="text-right shrink-0">
                        <div className="font-serif text-lg font-semibold" style={{ color: '#1C3A2A' }}>{p.views}</div>
                        <div className="text-xs" style={{ color: '#8A7F74' }}>views</div>
                      </div>
                    )}
                  </div>
                </div>
            ...
            >
              <div className="w-14 h-14 rounded-full flex items-center justify-center text-2xl transition-transform duration-300 group-hover:scale-110"
                style={{ background: '#F5EDD9' }}>+</div>
              <div className="text-center">
                <p className="font-serif font-semibold text-base mb-1" style={{ color: '#1C3A2A' }}>New Memory</p>
                <p className="text-sm" style={{ color: '#8A7F74' }}>Start a new beautiful memory site</p>
              </div>
            </button>
          </div>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-2xl rounded-3xl overflow-hidden animate-scaleIn"
        style={{ background: '#FAF7F2', boxShadow: '0 40px 100px rgba(28,58,42,0.25)', border: '1px solid rgba(201,168,76,0.15)' }}>
        <div className="p-8">
          {/* Step indicator */}
          <div className="flex items-center justify-between mb-8">
            <h2 className="font-serif text-2xl font-semibold" style={{ color: '#1C3A2A' }}>New Memory</h2>
                ...
                <div key={s} className="flex items-center gap-2">
                  <div className="flex flex-col items-center">
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold transition-all duration-300"
                      style={{
                        background: i + 1 <= step ? '#1C3A2A' : '#F5EDD9',
                        color: i + 1 <= step ? '#FAF7F2' : '#8A7F74',
                      }}>
                      {i + 1 < step ? '✓' : i + 1}
                    </div>
                    <span className="text-xs mt-1 hidden sm:block" style={{ color: i + 1 === step ? '#1C3A2A' : '#8A7F74' }}>{s}</span>
                  </div>
                  {i < steps.length - 1 && (
                    <div className="w-8 h-px mb-4" style={{ background: i + 1 < step ? '#C9A84C' : 'rgba(28,58,42,0.1)' }} />
                  )}
                </div>
              ))}
            </div>
          ...
          {step === 1 && (
            <div>
              <p className="text-sm mb-5" style={{ color: '#8A7F74' }}>Choose a template to get started</p>
              <div className="grid grid-cols-2 gap-3">
                {templateOptions.map((t, i) => (
                  <button
                    ...
                    style={{
                      border: selected === i ? '2px solid #C9A84C' : '2px solid rgba(28,58,42,0.08)',
                  ...
                  >
                    ...
                    <div className="px-3 py-2.5">
                      <div className="text-sm font-medium" style={{ color: '#1C3A2A' }}>{t.name}</div>
                    </div>
                  </button>
                ))}
              </div>
          ...
          {step === 3 && (
            <div>
              <p className="text-sm mb-5" style={{ color: '#8A7F74' }}>Choose your aesthetic</p>
              <div className="grid grid-cols-3 gap-3">
                {[
                  { name: 'Warm & Cozy', colors: ['#F5EDD9', '#C9A84C', '#1C3A2A'] },
                  { name: 'Romantic Rose', colors: ['#FAF0F2', '#B76E79', '#2A2A2A'] },
                  { name: 'Garden Fresh', colors: ['#EFF5EF', '#2D5A3F', '#C9A84C'] },
                  { name: 'Midnight Blue', colors: ['#0F1923', '#4A90D9', '#FAF7F2'] },
                  ...
                  >
                    ...
                    <div className="px-2.5 py-2">
                      <span className="text-xs font-medium" style={{ color: '#1C3A2A' }}>{theme.name}</span>
                    </div>
                  </button>
                ))}
              </div>
...
function EditorView({ onBack, onPublish }: { onBack: () => void; onPublish: () => void }) {
  ...
  return (
    <div className="flex flex-col h-screen" style={{ background: '#1C3A2A' }}>
      {/* Editor toolbar */}
      <header className="flex items-center justify-between px-4 py-3 shrink-0"
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(201,168,76,0.2)', color: '#C9A84C' }}>Draft</span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
            ...
            {sections.map(s => (
              <button
                ...
                style={{
                  ...
                  color: activeSection === s.id ? '#C9A84C' : 'rgba(250,247,242,0.6)',
                }}
        ...
        <main className="flex-1 overflow-y-auto p-6 flex items-start justify-center" style={{ background: '#0F2318' }}>
          <div className="w-full max-w-2xl rounded-2xl overflow-hidden"
            style={{ background: '#FAF7F2', boxShadow: '0 20px 80px rgba(0,0,0,0.5)', minHeight: 600 }}>
            {/* Page header */}
            <div className="relative overflow-hidden" style={{ height: 200, background: 'linear-gradient(135deg, #1C3A2A, #2D5A3F)' }}>
            ...
            <div className="p-6">
              {activeSection === 'gallery' && (
                <div>
                  <h3 className="font-serif text-xl font-semibold mb-4" style={{ color: '#1C3A2A' }}>Memories Gallery</h3>
                  ...
                  <div className="mt-4 border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors duration-200"
                    style={{ borderColor: 'rgba(28,58,42,0.15)', color: '#8A7F74' }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = '#C9A84C'; e.currentTarget.style.background = 'rgba(201,168,76,0.04)' }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(28,58,42,0.15)'; e.currentTarget.style.background = 'transparent' }}>
              ...
              {activeSection === 'journal' && (
                <div>
                  <h3 className="font-serif text-xl font-semibold mb-4" style={{ color: '#1C3A2A' }}>Journal Pages</h3>
                  <div className="relative" style={{ minHeight: 300 }}>
                    <div className="rounded-xl p-6 paper-texture"
                      ...
                      <div className="absolute top-4 right-4">
                        <div className="tape text-xs px-3 py-1 rounded font-medium" style={{ color: '#8A7F74' }}>Page 1</div>
                      </div>
                    ...
                    <div className="flex gap-3 mt-4">
                      ...
                      <div className="flex gap-1 ml-auto items-center">
                        <button onClick={() => setCurrentPage(Math.max(0, currentPage - 1))} className="w-7 h-7 rounded-full flex items-center justify-center text-xs transition-colors"
                          style={{ background: '#F5EDD9', color: '#1C3A2A' }}>←</button>
                        <span className="text-xs px-3" style={{ color: '#8A7F74' }}>1 / 3</span>
              ...
              {activeSection === 'letter' && (
                <div>
                  <h3 className="font-serif text-xl font-semibold mb-4" style={{ color: '#1C3A2A' }}>Personal Letter</h3>
                  <div className="rounded-2xl overflow-hidden" style={{ boxShadow: '0 8px 40px rgba(0,0,0,0.1)' }}>
                    <div className="h-3" style={{ background: 'linear-gradient(90deg, #B76E79, #C9A84C, #8FAF8F)' }} />
                    <div className="p-8 paper-texture" style={{ background: '#FFFEF8' }}>
                      ...
                      <p className="font-serif text-base mt-6" style={{ color: '#1C3A2A', fontStyle: 'italic' }}>All my love, always — your daughter, Elena ♥</p>
                      <div className="mt-6 flex justify-end">
                        <div className="animate-wax-seal w-12 h-12 rounded-full flex items-center justify-center text-xl"
                          style={{ background: 'linear-gradient(135deg, #B76E79, #9A4F5A)', boxShadow: '0 4px 12px rgba(183,110,121,0.4)' }}>
                          ♥
                        </div>
                      </div>
                    </div>
              ...
              {activeSection === 'audio' && (
                <div>
                  <h3 className="font-serif text-xl font-semibold mb-4" style={{ color: '#1C3A2A' }}>Music & Audio</h3>
                  {/* Vinyl player */}
                  <div className="flex items-center gap-6 p-6 rounded-2xl mb-6"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="w-full h-1.5 rounded-full mb-2" style={{ background: 'rgba(255,255,255,0.1)' }}>
                        <div className="h-full rounded-full" style={{ width: '35%', background: 'linear-gradient(90deg, #C9A84C, #E2C97E)' }} />
                      </div>
                  ...
                  <div className="flex items-center gap-1 justify-center h-10 mb-4">
                    {Array.from({ length: 28 }).map((_, i) => (
                      <div key={i} className="w-1.5 rounded-full"
                        style={{
                          background: playing ? '#C9A84C' : '#C5D9C5',
              ...
              {(activeSection === 'timeline' || activeSection === 'settings') && (
                <div className="py-8 text-center">
                  ...
                  <h3 className="font-serif text-xl font-semibold mb-2" style={{ color: '#1C3A2A' }}>
        ...
        <aside className="w-52 shrink-0 flex flex-col overflow-y-auto" style={{ background: '#162E20', borderLeft: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="p-4 space-y-4">
            ...
            {[
              { label: 'Background', type: 'color', val: '#FAF7F2' },
              { label: 'Accent', type: 'color', val: '#C9A84C' },
            ].map(prop => (
              <div key={prop.label}>
                ...
                {['Fade', 'Float', 'Slide', 'Flip'].map(a => (
                  <button key={a} className="text-xs py-1.5 rounded-lg transition-colors"
                    style={{ background: a === 'Fade' ? 'rgba(201,168,76,0.2)' : 'rgba(255,255,255,0.05)',
                      color: a === 'Fade' ? '#C9A84C' : 'rgba(250,247,242,0.5)',
              ...
              {['Public', 'Private', 'Invite only'].map(v => (
                <label key={v} className="flex items-center gap-2 py-1.5 cursor-pointer">
                  <div className="w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center"
                    style={{ borderColor: v === 'Public' ? '#C9A84C' : 'rgba(255,255,255,0.2)' }}>
                    {v === 'Public' && <div className="w-1.5 h-1.5 rounded-full" style={{ background: '#C9A84C' }} />}
                  </div>
...
function PublishView({ onBack }: { onBack: () => void }) {
  ...
  const confettiItems = Array.from({ length: 30 }, (_, i) => ({
    left: `${Math.random() * 100}%`,
    color: ['#C9A84C', '#B76E79', '#8FAF8F', '#C5B8D4', '#F2C4A0'][i % 5],
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 relative overflow-hidden"
      style={{ background: 'linear-gradient(135deg, #FAF7F2 0%, #F5EDD9 50%, #EFF5EF 100%)' }}>
      ...
      <div className="relative z-10 w-full max-w-lg text-center animate-scaleIn">
        ...
        <h1 className="font-serif text-4xl font-semibold mb-3" style={{ color: '#1C3A2A' }}>
          Your memory is live!
        </h1>
        <p className="text-base mb-8" style={{ color: '#8A7F74', fontWeight: 300 }}>
        ...
        <div className="flex items-center gap-2 p-2 rounded-2xl mb-4"
          ...
          <div className="flex-1 px-3 py-2 text-sm font-medium text-left truncate" style={{ color: '#1C3A2A' }}>
            🔗 {link}
          </div>
          <button
            onClick={handleCopy}
            ...
            style={{
              background: copied ? '#2D5A3F' : 'linear-gradient(135deg, #1C3A2A, #2D5A3F)',
              color: copied ? '#C9A84C' : '#FAF7F2',
            }}
          >
        ...
        <div className="flex justify-center mb-8">
          <div className="p-4 rounded-2xl bg-white" style={{ boxShadow: '0 4px 20px rgba(28,58,42,0.08)' }}>
            <div className="w-24 h-24 rounded-xl overflow-hidden" style={{ background: '#1C3A2A' }}>
              <svg viewBox="0 0 24 24" fill="none" className="w-full h-full p-2">
                <rect x="2" y="2" width="8" height="8" rx="1" fill="#C9A84C" />
                <rect x="4" y="4" width="4" height="4" rx="0.5" fill="#1C3A2A" />
                <rect x="14" y="2" width="8" height="8" rx="1" fill="#C9A84C" />
                <rect x="16" y="4" width="4" height="4" rx="0.5" fill="#1C3A2A" />
                <rect x="2" y="14" width="8" height="8" rx="1" fill="#C9A84C" />
                <rect x="4" y="16" width="4" height="4" rx="0.5" fill="#1C3A2A" />
                <rect x="14" y="14" width="3" height="3" rx="0.5" fill="#C9A84C" />
                <rect x="19" y="14" width="3" height="3" rx="0.5" fill="#C9A84C" />
                <rect x="14" y="19" width="3" height="3" rx="0.5" fill="#C9A84C" />
                <rect x="19" y="19" width="3" height="3" rx="0.5" fill="#C9A84C" />
              </svg>
            </div>
            <p className="text-xs text-center mt-2" style={{ color: '#8A7F74' }}>Scan to open</p>
          </div>
        </div>
        ...
        <div className="flex gap-3 justify-center mb-8">
          {['📱 WhatsApp', '📘 Facebook', '🐦 Twitter', '📧 Email'].map(s => (
            <button key={s} className="text-xs font-medium px-4 py-2.5 rounded-xl transition-all duration-200"
              style={{ background: 'white', border: '1px solid rgba(28,58,42,0.1)', color: '#2A2A2A' }}
              onMouseEnter={e => { e.currentTarget.style.background = '#F5EDD9'; e.currentTarget.style.borderColor = 'rgba(201,168,76,0.3)' }}
        ...
        <div className="flex gap-3 justify-center">
          <button
            onClick={onBack}
            className="px-6 py-3 rounded-xl text-sm font-medium transition-all duration-200"
            style={{ border: '1px solid rgba(28,58,42,0.15)', color: '#1C3A2A' }}
            onMouseEnter={e => e.currentTarget.style.background = '#F5EDD9'}
            onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
          >
            Back to Dashboard
...
function LandingPage({ onStart, onAuth }: { onStart: () => void; onAuth: () => void }) {
  return (
    <div style={{ background: '#FAF7F2' }}>
      ...
      <FeaturesSection />
      <TemplatesSection onStart={onAuth} />
      <TestimonialsSection />
      <CTASection onStart={onAuth} />
      <Footer />
    </div>
  )
}
